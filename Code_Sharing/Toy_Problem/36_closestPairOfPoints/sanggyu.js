...
function calculateDistance(p1, p2) {
    //p1[1, 3], p2[2, 2] 이면
    const yDiffSquared = Math.pow(p2[0] - p1[0], 2); //yDiffSquared: 1
    const xDiffSquared = Math.pow(p2[1] - p1[1], 2); //xDiffSquared: -1
    const dist = Math.sqrt(yDiffSquared + xDiffSquared);  //Math.sqrt(2) 2에 제곱근 1.4142135623730951
  ...
  const merge = function (left, right, comparator = (item) => item) {
    ...
    for (let i = 0; i < size; i++) {
      //left의길이를 벗어났다는 것은 right의 인덱스라는 것
      if (leftIdx >= left.length) {
        merged.push(right[rightIdx]);
        rightIdx++;
      } else if (
        //위와 반대 && 오름차순 정렬이니까 왼쪽 요소가 더 작으면 먼저 넣는다
  ...
  //여기까지가 closestPairOfPoints->closestFrom 메서드에 들어갈 인자배열을 정렬하는 과정
  ...
  //closestFrom의 리턴에 사용될 메서드 2
    const closestCrossing = (mid, sorted, min) => {
      ...
      //각각 가장 가까운 두 점 사이의 거리
                                //sorted.length
    const closestFrom = (start, end, size, sorted) => {
      //base(재귀돌면서 여기로 탈출)
      if (size <= 3) {
        // 최소 두 개 이상의 점이 있어야 거리를 계산할 수 있다. 점 2개로 나눈다
